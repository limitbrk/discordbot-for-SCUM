    ...
    class Step1View(discord.ui.View):
        ...
        async def goto_step2(self, interaction: discord.Interaction, lang: str):
            ...
            await interaction.edit_original_response(
                embed=embed.register_desc(_("Please read these rule before continue...", lang), desc),
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if profile is None:
                ...
                await interaction.edit_original_response(
                    content=None,
                    embed=embed.error(
                        _("Invalid SteamID Please try again", self.lang)
                    ),
                    view=None
                )
                return
            steam_embed = embed.steam_info(
                _("This is your Steam?", self.lang),
                profile
            )
            await interaction.response.defer()
            await interaction.edit_original_response(
                content=None,
                embed=steam_embed,
                view=Register.Step4View(self.lang, profile)
            )
